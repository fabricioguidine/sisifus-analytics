...
from src.config import ANALYTICS_CSV, ANALYTICS_JSON, SANKEY_HTML
...
class AnalyticsGenerator:
                ...
                try:
                    ...
                    if date:
                        if (
                            ...
                            or date < stats["date_range"]["earliest"]
                        ):
                            stats["date_range"]["earliest"] = date
                        if (
                            not stats["date_range"]["latest"]
                            or date > stats["date_range"]["latest"]
                        ):
    ...
    def calculate_accuracy(self) -> float:
        ...
        high_confidence_count = sum(
            1 for email in self.emails if email.get("confidence", 0.0) > 0.5
        )
    ...
    def generate_summary(self) -> Dict:
        ...
        job_related_emails = [
            email for email in self.emails if email.get("status") not in excluded_statuses
        ]
        ...
        summary = {
            ...
            "total_companies": len(
                [
                    c
                    for c, data in self.stats["by_company"].items()
            ...
            "date_range": {
                ...
                if self.stats["date_range"]["latest"]
                else None,
            },
    ...
    def _calculate_accuracy_for_emails(self, emails: List[Dict]) -> float:
        ...
        high_confidence_count = sum(1 for email in emails if email.get("confidence", 0.0) > 0.5)
    ...
    def _get_company_flow(self) -> Dict[str, Dict[str, Any]]:
        ...
        for email_data in self.emails:
            ...
            elif status == "rejected" and flow["final_status"] not in [
                "accepted",
                "offer",
                "withdrew",
            ]:
    ...
    def generate_sankey_diagram(self) -> go.Figure:
        ...
        total_rejected = flow_counts["rejected_direct"] + sum(
            flow_counts["rejected_from_interview"].values()
        )
        total_withdrew = flow_counts["withdrew_direct"] + sum(
            flow_counts["withdrew_from_interview"].values()
        )
        total_ghosted = flow_counts["ghosted_direct"] + sum(
            flow_counts["ghosted_from_interview"].values()
        )
        ...
        if (
            flow_counts["offer"] > 0
            or flow_counts["accepted"] > 0
            or flow_counts["declined_offer"] > 0
        ):
            total_offers = (
                flow_counts["offer"] + flow_counts["accepted"] + flow_counts["declined_offer"]
            )
        ...
        fig.update_layout(
            title_text="Job Application Flow - Sankey Diagram", font_size=12, height=800
        )
    ...
    def save_analytics(self):
        ...
        summary["company_details"] = {
            company: {"count": data["count"], "statuses": Counter(data["statuses"])}
