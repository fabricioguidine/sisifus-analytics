...
def main():
    ...
    if args.use_input:
        if args.extract_only:
            ...
            print(
                "Use --extract-only to fetch and save emails, or --use-input to load saved emails."
            )
            sys.exit(1)
        ...
        emails = load_emails_from_input(
            months=args.months, year=args.year, prompt_if_missing=prompt_for_filter
        )
        if not emails:
            sys.exit(1)
    else:
